...
export default function SettingSupportButton({
    ...
    return (
        <button
            className='w-full flex items-center justify-between py-3 group transition-colors hover:bg-slate-700/50 rounded-lg px-2 -mx-2'
            onClick={moveSubSettingPage}>
            <div className='text-left'>
                <p className='font-medium'>{settingName}</p>
                {(settingDescription !== null) &&
                    <p className='text-slate-400 text-sm'>{settingDescription}</p>}
            </div>
